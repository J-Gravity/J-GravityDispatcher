...
/*   Updated: 2017/06/23 14:25:14 by cyildiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_event(t_worker *worker, t_msg msg)
{
	if (msg.id == WORK_UNITS_READY)
	{
		...
		send_msg(worker->socket.fd, (t_msg){WORK_UNIT_REQUEST, 1, _strdup(" ")});
		free(msg.data);
	}
	...
	else if (msg.id == WORK_UNIT)
	{
		...
		ReleaseSemaphore(worker->debundle_sem, 1, NULL);
		if (DEBUG)
...
static void	*event_thread(void *param)
{
	...
	while (worker->active)
	{
		...
		G_total_event_time += time(NULL) - event_time;
	}
	printf("EVENT- exiting event thread\n");
	ReleaseSemaphore(worker->exit_sem, 1, NULL);
	return (0);
}
